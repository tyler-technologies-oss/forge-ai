...
export function BudgetCard(ctx: ComponentContext<BudgetCardProps>): ReactElement {
  ...
  return (
    <ForgeCard
      className={`genui-budget-card ${isOverBudget ? 'genui-budget-card--over' : ''}`}
      onClick={() => ctx.emit('click', { title, percentUsed: Math.round(percentage) })}>
      ...
      {showRemaining && (
        <div className="genui-budget-card__footer">
          <span
            className={`genui-budget-card__remaining forge-typography--caption ${isOverBudget ? 'genui-budget-card__remaining--over' : ''}`}>
            {isOverBudget
