...
/**
 * The host-side bridge surface `mcp-app-bridge.ts` (`McpAppBridge`) implements. The
 * element + controller depend on this interface only — no ext-apps types leak through
 * it. `ext-app-bridge.ts` (`ExtAppsBridge`, wraps `@modelcontextprotocol/ext-apps`) is
 * orphaned but kept around for reference; it also implements this interface.
 */
...
export interface IMcpAppBridge {
  /**
