...
const BASE_GITHUB_URL = `https://github.com/tyler-technologies-oss/forge-ai/tree`;
...
function ForgeTypeLinks({ typeText }: { typeText: string }) {
  ...
  tokenizedType.forEach(token => {
    ...
    if (matchingType) {
      ...
      const href = `${BASE_GITHUB_URL}/${branchName}/packages/ai/${path}#L${lineNumber}`;
...
export default function CustomArgTypes() {
  ...
  const GitHubLink = ({ modulePath, className }: { modulePath: string; className?: string }) => (
    <a
      href={`${BASE_GITHUB_URL}/${branch}/packages/ai/${modulePath}`}
