/**
 * ForgeGenUIRenderer Component
 *
 * A Lit element that renders a GenUISpec into actual DOM.
 *
 * How it works:
 * 1. Set spec property with a GenUISpec (typically from LLM tool output)
 * 2. Set registry property with component factories
 * 3. Component recursively renders from spec.root, looking up each element's
 *    type in the registry and calling its LitTemplateFactory
 * 4. Props with { $state: "/path" } are resolved to actual state values
 * 5. User interactions call emit() which dispatches forge-genui-action events
 *
...
import type { LitRegistry, LitComponentContext } from './types.js';

import styles from './renderer.scss?inline';

@customElement('forge-genui-renderer')
export class ForgeGenUIRenderer extends LitElement {
  public static override styles = unsafeCSS(styles);

  @property({ attribute: false })
  public spec: GenUISpec | null = null;

  @property({ attribute: false })
  public registry: LitRegistry | null = null;
  ...
  #emit = (action: string, payload?: Record<string, unknown>): void => {
    const event = new CustomEvent<ActionEvent>('forge-genui-action', {
      bubbles: true,
      composed: true,
      detail: {
        action,
  ...
  #renderElement(elementId: string): TemplateResult | typeof nothing {
    ...
    if (element.visible && !this.#evaluateVisibility(element.visible as VisibilityCondition)) {
      return nothing;
    }
...
declare global {
  interface HTMLElementTagNameMap {
    'forge-genui-renderer': ForgeGenUIRenderer;
  }
}
