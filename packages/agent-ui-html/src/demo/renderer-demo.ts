...
@customElement('agent-ui-renderer-demo')
export class AgentUIRendererDemo extends LitElement {
  @state() private _state = INITIAL_STATE;
  @state() private _currentDataset: 'initial' | 'alternate' = 'initial';
  ...
  protected override render(): TemplateResult {
    return html`
      <forge-scaffold>
        <forge-app-bar slot="header" title-text="Renderer Demo (HTML)">
          <forge-icon slot="logo" name="forge_logo"></forge-icon>
        </forge-app-bar>

        <main slot="body" class="renderer-demo">
          <forge-card class="renderer-demo__controls">
            <h3>State Control</h3>
            <p>
              Demonstrates all expression types: <code>$state</code>, <code>$template</code>, <code>$cond</code>,
              <code>$bindState</code>, <code>$computed</code>, <code>watch</code>, <code>repeat</code>,
              <code>$item</code>, <code>$index</code>, <code>visible</code>, and <code>validation</code>.
            </p>
            <forge-button variant="raised" @click=${this.#toggleDataset}>
              Switch to ${this._currentDataset === 'initial' ? 'Alternate' : 'Initial'} Data
            </forge-button>
            <pre class="renderer-demo__state">${JSON.stringify(this._state, null, 2)}</pre>
          </forge-card>

          <div class="renderer-demo__output">
            <spec-renderer
              .spec=${DEMO_SPEC}
              .registry=${registry}
  ...
  #toggleDataset = (): void => {
    if (this._currentDataset === 'initial') {
      this._state = ALTERNATE_STATE;
      this._currentDataset = 'alternate';
    } else {
      this._state = INITIAL_STATE;
      this._currentDataset = 'initial';
    }
  };
}
...
declare global {
  interface HTMLElementTagNameMap {
    'agent-ui-renderer-demo': AgentUIRendererDemo;
  }
}
