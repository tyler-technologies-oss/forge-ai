...
`,status:"complete"},{type:"toolCall",data:{id:"tc-1",messageId:"res-1",name:"fetch_release",args:{channel:"stable"},status:"complete",type:"agent",result:{version:"2.4.0",date:"2026-06-20"},startTimestamp:Date.now()-1e4,endTimestamp:Date.now()-9200}},{type:"toolCall",data:{id:"tc-2",messageId:"res-1",name:"search_documentation",args:{query:"release notes 2.4.0"},status:"complete",type:"client",result:{found:!0,sections:3},startTimestamp:Date.now()-9200,endTimestamp:Date.now()-8500}},{type:"text",messageId:"res-1",content:"The latest stable release is **2.4.0** (June 20, 2026), which focuses on tooling improvements.",status:"complete"}]}}],n=new Map([["search_documentation",{name:"search_documentation",displayName:"Search Documentation",description:"Search through documentation"}],["fetch_release",{name:"fetch_release",displayName:"Fetch Release",description:"Fetch the latest release information"}]]),a=e("forge-ai-message-thread-copy"),o=e("forge-ai-message-thread-resend"),r=e("forge-ai-message-thread-thumbs-up"),i=e("forge-ai-message-thread-thumbs-down"),m=e("forge-ai-message-thread-scroll-request");return g`
      <div
        ...
        <forge-ai-message-thread
          ...
          @forge-ai-message-thread-copy=${a}
    ...
    `}},d={},l={render:t=>{const s=[{type:"message",data:{id:"msg-1",role:"user",content:"How many permits are overdue, and which departments are they stuck in?",timestamp:Date.now()-2e4,status:"complete"}},{type:"assistant",data:{id:"res-1",status:"complete",timestamp:Date.now()-15e3,children:[{type:"text",messageId:"res-1",content:`Let me check the current backlog. First I'll pull every pending permit, then narrow it down to the ones past their SLA.
...
Building and Zoning together account for roughly 70% of the backlog, so they're the best place to focus.`,status:"complete"}]}}],n=new Map([["permits.query",{name:"permits.query",displayName:"Query Permits",description:"Pull permits matching a status and SLA window"}],["permits.group_by",{name:"permits.group_by",displayName:"Group Permits",description:"Break the backlog down by a field"}]]),a=e("forge-ai-message-thread-copy"),o=e("forge-ai-message-thread-resend"),r=e("forge-ai-message-thread-thumbs-up"),i=e("forge-ai-message-thread-thumbs-down"),m=e("forge-ai-message-thread-scroll-request");return g`
      <div
        style="width: 100%; height: 600px; max-width: 800px; margin: 0 auto; border: 1px solid #ccc; overflow: hidden;">
        <forge-ai-message-thread
          .messageItems=${s}
...
`,status:"complete"},{type:"toolCall",data:{id:"tc-1",messageId:"res-1",name:"sales.fetch_report",args:{period:"Q2-2026",region:"all"},status:"complete",type:"agent",result:{total:482e4,currency:"USD",rows:128},startTimestamp:Date.now()-15e3,endTimestamp:Date.now()-14500}},{type:"toolCall",data:{id:"tc-2",messageId:"res-1",name:"email.send",args:{list:"leadership",subject:"Q2 2026 Sales"},status:"error",type:"agent",result:{error:'SMTP 550: recipient list "leadership" not found',code:550},startTimestamp:Date.now()-14500,endTimestamp:Date.now()-13900}},{type:"text",messageId:"res-1",content:"I pulled the Q2 2026 report (**$4.82M** across 128 rows), but sending the email **failed** — the mailing list `leadership` could not be found. Could you confirm the correct list name or share the recipient addresses?",status:"complete"}]}}],n=new Map([["sales.fetch_report",{name:"sales.fetch_report",displayName:"Fetch Sales Report",description:"Pull an aggregated sales report for a period"}],["email.send",{name:"email.send",displayName:"Send Email",description:"Send an email to a distribution list"}]]),a=e("forge-ai-message-thread-copy"),o=e("forge-ai-message-thread-resend"),r=e("forge-ai-message-thread-thumbs-up"),i=e("forge-ai-message-thread-thumbs-down"),m=e("forge-ai-message-thread-scroll-request");return g`
    ...
    `}},p={render:t=>{const s=e("forge-ai-message-thread-copy"),n=e("forge-ai-message-thread-resend"),a=e("forge-ai-message-thread-thumbs-up"),o=e("forge-ai-message-thread-thumbs-down"),r=e("forge-ai-message-thread-scroll-request"),i=e("client-message-action-click"),m=[{id:"refresh",label:"Refresh",onClick:()=>i("refresh")}],u=[{type:"message",data:{id:"msg-1",role:"user",content:"Summarize the latest incident report.",timestamp:Date.now()-9e4,status:"complete"}},{type:"message",data:{id:"switch-1",role:"system",content:"Switched to Incident Response Agent",timestamp:Date.now()-85e3,status:"complete",clientOnly:!0}},{type:"message",data:{id:"msg-2",role:"assistant",content:"Here is a summary of the most recent incident report...",timestamp:Date.now()-8e4,status:"complete"}},{type:"message",data:{id:"indexing-1",role:"system",content:"Indexing 3 uploaded documents...",timestamp:Date.now()-6e4,status:"complete",clientOnly:!0,kind:"info",header:"Indexing"}},{type:"message",data:{id:"upload-error-1",role:"system",content:"incident-photos.zip could not be uploaded because it exceeds the 25 MB limit.",timestamp:Date.now()-5e4,status:"complete",clientOnly:!0,kind:"warning",header:"Upload failed"}},{type:"message",data:{id:"msg-3",role:"user",content:"Send a message now.",timestamp:Date.now()-3e4,status:"complete"}},{type:"message",data:{id:"session-expired-1",role:"system",content:"Your session expired. Please refresh the page to re-authenticate.",timestamp:Date.now()-25e3,status:"complete",clientOnly:!0,kind:"error",header:"Session expired",actions:m}},{type:"message",data:{id:"upload-success-1",role:"system",content:"incident-photos-resized.zip uploaded successfully.",timestamp:Date.now()-1e4,status:"complete",clientOnly:!0,kind:"success"}}];return g`
      <div
        style="width: 100%; height: 600px; max-width: 800px; margin: 0 auto; border: 1px solid #ccc; overflow: hidden;">
        <forge-ai-message-thread
          .messageItems=${u}
          ...
          ?debug-mode=${t.debugMode}
          auto-scroll=${t.autoScroll}
          @forge-ai-message-thread-copy=${s}
          @forge-ai-message-thread-resend=${n}
          @forge-ai-message-thread-thumbs-up=${a}
          @forge-ai-message-thread-thumbs-down=${o}
          @forge-ai-message-thread-scroll-request=${r}>
        </forge-ai-message-thread>
      </div>
...
}`,...p.parameters?.docs?.source}}};const f=["Demo","WithToolCalls","WithErroredToolCalls","WithClientMessages"],D=Object.freeze(Object.defineProperty({__proto__:null,Demo:d,WithClientMessages:p,WithErroredToolCalls:c,WithToolCalls:l,__namedExportsOrder:f,default:y},Symbol.toStringTag,{value:"Module"}));export{D as A,d as D,p as W};
